...
<div class="mb-[30px]">
    ...
    <div
        class="flex {{ $type === 'textarea' ? 'items-start' : 'items-center' }} border-2 border-[#5F5F5F] rounded-md p-2">
        ...
        <input type="{{ $type }}" id="{{ $name }}" name="{{ $name }}" placeholder="{{ $placeholder }}"
            value="{{ old($name, $value) }}" {{ $attributes->merge(['class' => 'w-full h-[30px] text-[18px]
        focus:outline-none']) }}>
        @endif
    </div>
